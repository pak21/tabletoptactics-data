...
def test_extract_armies_from_slug_extracts_factions(showdatabuilder, factions, subfactions):
    slug = 'space-marines-vs-chaos-space-marines-warhammer-40000-battle-report'

    armies = showdatabuilder.extract_armies_from_slug(slug, factions, subfactions)
...
def test_extract_armies_from_slug_extracts_subfactions(showdatabuilder, factions, subfactions):
...
def test_extract_armies_from_slug_recognises_world_eaters_as_a_faction(showdatabuilder, factions, subfactions):
...
def test_extract_armies_from_slug_recognises_slaves_to_darkness_as_a_faction(showdatabuilder, factions, subfactions):
...
def test_extract_armies_from_slug_throws_exception_if_no_armies_found(showdatabuilder, factions, subfactions):
    slug = 'starfleet-vs-klingons-star-trek-battle-report'

    with pytest.raises(Exception):
        armies = showdatabuilder.extract_armies_from_slug(slug, factions, subfactions)

def test_extract_armies_from_slug_extracts_one_army(showdatabuilder, factions, subfactions):
...
def test_extract_armies_from_slug_throws_exception_if_three_armies_found(showdatabuilder, factions, subfactions):
    slug = 'dark-angels-vs-emperors-children-vs-world-eaters-warhammer-40000-mega-battle-report'

    with pytest.raises(Exception):
        armies = showdatabuilder.extract_armies_from_slug(slug, factions, subfactions)

def test_extract_armies_from_slug_handles_chaos_space_marines(showdatabuilder, factions, subfactions):
...
def test_get_game_parses_correctly(showdatabuilder, games):
    slug = 'space-marines-vs-orks-warhammer-40000-boarding-action-battle-report'

    game_id, game = showdatabuilder.get_game(slug, games, None)

    assert game_id == 1
    assert game == 'Warhammer 40,000'

def test_get_game_has_special_case_for_40k(showdatabuilder, games):
...
def test_get_game_uses_missing_value_if_unmatched(showdatabuilder, games):
    slug = 'space-marines-vs-orks-boarding-action-battle-report'

    game_id, game = showdatabuilder.get_game(slug, games, 'Warhammer 40,000')

    assert game_id == 1

def test_get_game_throws_exception_if_no_value(showdatabuilder, games):
    slug = 'space-marines-vs-orks-boarding-action-battle-report'

    with pytest.raises(Exception):
        showdatabuilder.get_game(slug, games, None)

def test_get_showtype_parses_correctly(showdatabuilder, showtypes):
    slug = 'aeldari-vs-imperial-knights-warhammer-40k-battle-report'
...
def test_get_showtype_uses_missing_value_if_unmatched(showdatabuilder, showtypes):
    slug = 'aeldari-vs-imperial-knights-warhammer-40k'

    showtype_id = showdatabuilder.get_showtype(slug, showtypes, 'Battle report')
...
def test_update_army_sets_army1_player(showdatabuilder, players, factions, subfactions):
    old_army = tt.ArmyInfo(faction_id=1, faction='Drukhari')
    input_data = tt.InputData(army1player='Spider')

    new_army = showdatabuilder.update_army_info(old_army, input_data, 1, players, factions, subfactions)

    assert new_army.player_id == 1

def test_update_army_set_army2_player(showdatabuilder, players, factions, subfactions):
    old_army = tt.ArmyInfo(faction_id=1, faction='Drukhari')
    input_data = tt.InputData(army2player='Spider')

    new_army = showdatabuilder.update_army_info(old_army, input_data, 2, players, factions, subfactions)

    assert new_army.player_id == 1

def test_update_army_requires_player(showdatabuilder, players, factions, subfactions):
    old_army = tt.ArmyInfo(faction_id=1, faction='Drukhari')

    with pytest.raises(Exception):
        new_army = showdatabuilder.update_army_info(old_army, tt.InputData(), 1, players, factions, subfactions)

def test_update_army_does_not_require_player_if_none(showdatabuilder, players, factions, subfactions):
    new_army = showdatabuilder.update_army_info(None, tt.InputData(), 1, players, factions, subfactions)

    assert new_army is None

def test_update_army_sets_subfaction(showdatabuilder, players, factions, subfactions):
...
def test_update_army_creates_army(showdatabuilder, players, factions, subfactions):
    input_data = tt.InputData(army1player='Spider', army1faction='Chaos Space Marines')

    new_army = showdatabuilder.update_army_info(None, input_data, 1, players, factions, subfactions)

    assert new_army is not None
    assert new_army.faction_id == 2

def test_set_winner_does_nothing_if_not_specified(showdatabuilder, players):
    army1 = tt.ArmyInfo(faction_id=1, faction='Space Marines')
    army2 = tt.ArmyInfo(faction_id=2, faction='Tyranids')

    showdatabuilder.set_winner(army1, army2, tt.InputData(), players)

    assert army1.winner is None
    assert army2.winner is None

def test_sets_winner_if_specified(showdatabuilder, players):
    ...
    showdatabuilder.set_winner(army1, army2, input_data, players)
